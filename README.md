...
> **Status:** Phase 0 — foundation only. The title screen runs; the world, battle, and menu scenes come in later phases.
...
| 1 | Tile-based overworld, player movement, NPCs, dialogue, map transitions | ⏳ |
...
4. The title screen appears. **New Game** is wired but not functional yet (Phase 1 lands the world scene).
...
scenes/                  Godot scene files (.tscn)
  ui/TitleScreen.tscn    Working title screen (current main scene)
scripts/
  globals/               Autoloaded singletons (run from boot)
    ...
    SceneRouter.gd       Fade-to-black scene transitions
  resources/             Schema scripts for the .tres files in data/
  ui/, world/, battle/   Per-system scripts (filled out in later phases)
```
